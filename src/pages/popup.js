...
var disableAll = document.getElementById('disableAll'),
    version = "KitingKeys " + chrome.runtime.getManifest().version;
...
document.getElementById('reportIssue').addEventListener('click', function () {
    window.close();
    var description = "%23%23+Error+details%0A%0A{0}%0A%0AKitingKeys%3A+{1}%0A%0ABrowser%3A+{2}%0A%0AURL%3A+{3}%0A%0A%23%23+Context%0A%0A%2A%2APlease+replace+this+with+a+description+of+how+you+were+using+KitingKeys.%2A%2A".format(encodeURIComponent(""), chrome.runtime.getManifest().version, encodeURIComponent(navigator.userAgent), encodeURIComponent("<The_URL_Where_You_Find_The_Issue>"));
